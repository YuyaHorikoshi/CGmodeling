...
Object.assign( THREE.SEA3D.DomainManager.prototype, {
	...
	add: function ( domain ) {
		...
		this.ballsmaps = this.ballsmaps || domain.ballsmaps;
	...
	dispose: function () {
		...
		this.disposeList( this.ballsmaps );
...
THREE.SEA3D.prototype.readCubeRender = function ( sea ) {
	...
	var balls = new THREE.CubeCamera( 0.1, 5000, THREE.SEA3D.RTT_SIZE );
	balls.renderTarget.ballsCamera = balls;

	sea.tag = balls.renderTarget;

	this.domain.ballsRenderers = this.ballsRenderers = this.ballsRenderers || [];
	this.ballsRenderers.push( this.objects[ "rttc/" + sea.name ] = balls );

	this.addSceneObject( sea, balls );
	this.updateTransform( balls, sea );
...
THREE.SEA3D.prototype.readCubeMap = function ( sea ) {
	...
	for ( var i = 0; i < faces.length; ++ i ) {
		...
		var ballsImage = new Image();
		ballsImage.onload = onLoaded;
		ballsImage.src = this.createObjectURL( faces[ i ].buffer, "image/" + sea.extension );

		texture.images[ i ] = ballsImage;

	}

	if ( ! this.config.async ) this.file.resume = false;

	this.domain.ballsmaps = this.ballsmaps = this.ballsmaps || [];
	this.ballsmaps.push( this.objects[ "cmap/" + sea.name ] = sea.tag = texture );
	...
	if ( format == "hdr" ) {
		...
		texture = new THREE.HDRCubeTextureLoader().load( THREE.UnsignedByteType, faces, function ( texture ) {
			...
			if ( usePMREM ) {
				...
				var pmremCubeUVPacker = new THREE.PMREMCubeUVPacker( pmremGenerator.ballsLods );
