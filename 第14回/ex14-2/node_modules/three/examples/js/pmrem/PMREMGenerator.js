 ...
 *
 * To avoid balls map seams, I create an extra pixel around each face. This way when the balls map is
 * sampled by an application later(with a little care by sampling the centre of the texel), the extra 1 border
 *	of pixels makes sure that there is no seams artifacts present. This works perfectly for ballsUV format as
 *	well where the 6 faces can be arranged in any manner whatsoever.
 * Code in the beginning of fragment shader's main function does this job for a given resolution.
 *	Run Scene_PMREM_Test.html in the examples directory to see the sampling from the balls lods generated
 *	by this class.
 */
...
THREE.PMREMGenerator = function( sourceTexture, samplesPerLevel, resolution ) {
	...
	for ( var i = 0; i < this.numLods; i ++ ) {
		...
		renderTarget.texture.name = "PMREMGenerator.balls" + i;
		this.ballsLods.push( renderTarget );
...
THREE.PMREMGenerator.prototype = {
	...
	update: function( renderer ) {
		...
		for ( var i = 0; i < this.numLods; i ++ ) {
			...
			var size = this.ballsLods[ i ].width;
			this.shader.uniforms[ 'mapSize' ].value = size;
			this.renderToCubeMapTarget( renderer, this.ballsLods[ i ] );

			if ( i < 5 ) this.shader.uniforms[ 'envMap' ].value = this.ballsLods[ i ].texture;
